# AnthropicProxy 1.0.1
![](Images/AnthropicProxy.jpg)

Небольшой локальный прокси на C# / ASP.NET Core, который позволяет запустить `Claude Desktop` в `gateway`-режиме и перенаправить запросы в OpenRouter.  Данная штука позволит прикоснуться к довольно хайповой штуковине Cloude и "потрогать" её неочевидные фичи Cowork и Code вообще без затрат. Ненужность фирменной авторизации подходит в случае, если аккаунт забанен Антропиком или на вашем тарифном плане исчерпаны все лимиты для продолжения взаимодействия с Cloude. По сути, "супермашина" Cloude Desktop будет более-менее комфортно работать (без оглядки на лихое "выжирание кредитов") с альтернативным "движком" Openrouter с его прикольными free-моделями, не так уж и уступающими хвалёным фирменным моделькам Opus или Sonnet (от "движка" Anthropic). 
...
Version = new Version(1, 1) + VersionPolicy = RequestVersionExact — самое важное. .NET 8 пытается договориться об HTTP/2, OpenRouter при этом сбрасывает TCP-соединение (тот самый SocketException 10054). Явный HTTP/1.1 это лечит.
SslProtocols = Tls12 | Tls13 в HttpClientHandler — убирает зависимость от системных настроек TLS.
Вместо return Results.StatusCode(501) теперь полноценное SSE-проксирование через CopyToAsync — поток байт от OpenRouter передаётся клиенту напрямую без буферизации.
...
- тонкая совместимость работы фриварных моделек с `Claude Desktop` во вкладке Code 
- полноценная поддержка моделей-заменителей Haiku или Sonnet не реализована (сейчас, похоже, маппинг идет на "универсальную" модель "free", которая из-за "универсальности" плоха в плане скорости обработки запросов), из-за этого возникает ошибка 429! 

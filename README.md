...
Небольшой локальный прокси на C# / ASP.NET Core, который позволяет запустить `Claude Desktop` в `gateway`-режиме и перенаправить запросы в OpenRouter.  Данная штука позволит прикоснуться к довольно хайповой штуковине Cloude и "потрогать" её неочевидные фичи Cowork и Code вообще без затрат. Ненужность фирменной авторизации подходит в случае, если аккаунт забанен Антропиком. По сути, "супермашина" Cloude Desktop будет работать с альтернативным "движком" Openrouter с его прикольными free-моделями, не так уж и уступающими хвалёным Opus или Sonnet. 
